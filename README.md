...
Versión actual: **5.1.0**: las burbujas del chat privado se quitan deslizándolas a un lado y vuelven solas cuando el otro escribe (véase «La 5»). La 5.0.0 cierra la serie 5: la identidad **Plaza** entera, de la paleta y la vaca a la partida en el teléfono, el ordenador a tres columnas, las pantallas para volver cada día y el movimiento —véase «La serie 5: la identidad Plaza»—. La mayor sube porque lo decidió el plan de la serie y no porque se rompiera el API: el contrato del servidor sigue siendo el de la 4, con un solo añadido compatible —los puntos de la partida terminada en su estado—; lo que cambia de arriba abajo es la interfaz, que no conserva ni una pantalla de la 4.1.0. La 4.0.0 cierra «El camino a la 4.0.0»: dieciocho mejoras en seis etapas, ordenadas alrededor de una sola frase —quien llegaba creaba una cuenta, iba a buscar el correo, volvía, entraba al vestíbulo y no había nadie—. La mayor sube por tres razones concretas, no por ceremonia: hay pantallas que funcionan **sin sesión**, cuando hasta la 3.5.1 todo lo que no fuera el buzón exigía una; el **ranking cambió de forma**, porque dejó de ordenar por victorias y pasó a contar puntos por temporada, de modo que su respuesta ya no es la misma; y llegó un modo con **más de dos jugadores**, que el modelo de `games`, con sus `p1` y `p2`, no podía representar. Antes de ponerle el número se auditó entera —véase «La auditoría de la 4.0.0»—, porque dieciocho cambios en seis etapas dejan grietas en las costuras y no en el sitio donde se miró al escribirlas.
...
**Las burbujas de las conversaciones privadas** (5.1.0). En el vestíbulo del teléfono, cada conversación privada de las últimas 24 horas flota como una burbuja con la inicial del otro, encima del botón del chat —tres como mucho; las demás, en «+N»—. Se quita deslizándola a un lado, hacia cualquiera de los dos, o con el botón «Quitar la burbuja» de la cabecera de su conversación; con el teclado, Suprimir o Retroceso sobre la burbuja. Vuelve sola cuando el otro escribe —un mensaje, un zumbido o una reacción—, y no por los avisos de la partida (entró, terminó, revancha), que no tienen autor: jugar otra partida contra la misma persona no la hace volver. El punto rojo sigue la misma regla: se enciende por lo que escribe el otro, no por cada partida terminada. Lo que se guarda es el último mensaje que había al quitarla, por hilo y en `localStorage` (`pf_chat_hidden`): quitar una burbuja no escribe en D1, y `listThreads` devuelve `lastSender`, que la consulta ya leía, para saber quién escribió lo último. La conversación no se pierde: en el teléfono el chat del vestíbulo lleva también las pestañas de las conversaciones privadas, con las que ya no tienen burbuja, y es el camino de vuelta. El vestíbulo deja debajo el sitio que ocupan el botón del chat y las burbujas (`--burbujas`), para que su pie no quede tapado.
...
- `tools/simulate-players.mjs`: simula a ocho personas jugando a la vez contra el Worker en Miniflare —uniones, intentos, revancha, arena, chat, sondeo, código del día, reloj, Cron y peticiones malformadas— y comprueba los invariantes del servidor. Se ejecuta con `npm run simulate`; véase «La auditoría de la 5».
...
Cada color de marca tiene su variante oscura para el reborde (`--azul-2`, `--coral-2`…), su fondo suave (`--azul-suave`…) y, cuando hace falta contraste sobre fondo claro, su tono de texto (`--azul-texto`, `--fija-texto`, `--pica-texto`). Los nombres de **Mesa** (`--ink`, `--panel`, `--edge`, `--text`, `--muted`, `--accent`, `--pink`) vivieron como alias de los nuevos durante la serie y se retiraron en la 5.0.0: los dos últimos que los pedían eran el código de la sala de espera y el de «Unirse a», que ahora piden `--azul-texto`. `/admin` lleva su propia hoja y sus propios nombres, que no son alias de nada.
...
npm run simulate    # ocho jugadores a la vez contra un Worker local, sin red
npm run db:local
npm run db:remote
```
...
## La auditoría de la 5

Cerrada la serie Plaza, el 23 de septiembre de 2026 se auditó lo entregado contra el lienzo validado y se sometió el servidor a varias personas jugando a la vez. Son tres trabajos distintos y se cuentan por separado.

**Lo que se comprobó contra el lienzo.** Barrido estático del código: ningún color de la época Mesa, ninguna de las dos tipografías anteriores, ningún alias `--ink`/`--accent` fuera de `public/admin.html` —que conserva su paleta propia, oscura, porque es una herramienta interna que nadie ve desde el juego— y ningún emoji en títulos, botones ni avisos; quedan los glifos tipográficos (`✓`, `⌫`, `✦`) y el contenido del chat. Los tres catálogos tienen las mismas 741 claves con los mismos marcadores. Barrido en el servidor local, con sesión, de doce pantallas de consulta en español, inglés y francés a 375, 768, 1009 y 1440 px, buscando la página que se desborda de lado, el texto que se sale de su caja y la clave que se enseña sin traducir: nada. Revisadas a ojo, de día y de noche: el vestíbulo del ordenador con su raíl y sus tres columnas, la partida en teléfono con la cabecera fija, el muelle y el teclado plegable, la partida en ordenador con los dos diarios y el chat anclado, la tarjeta de fin con la vaca y el confeti, el código del día, el podio, el perfil con sus insignias dibujadas, los enigmas y la arena. Coinciden con el lienzo.

**Lo que se dejó fuera a propósito y sigue fuera**, para que nadie lo busque como un olvido: la lista de gente conectada y el podio pequeño junto al vestíbulo del ordenador, el puesto de temporada y la presencia en el perfil, la racha y la barra de temporada en la tarjeta de fin —todos pedirían lecturas que el sondeo no hace, y D1 Free manda—; el interruptor de tema, porque el modo noche sigue al sistema y `data-theme` queda preparado para el día en que exista; y la paleta de `/admin`.

**Lo que encontró.** Una sola cosa, de antes de la serie: las burbujas de los hilos privados se quedaban flotando fuera del vestíbulo y tapaban el teclado del código del día y dos enigmas en un teléfono (5.0.2).

**La simulación.** `npm run simulate` levanta el Worker con Miniflare y una base vacía, entra con ocho cuentas y encadena trece escenarios de concurrencia: tres que se unen a la misma partida en el mismo instante; seis intentos a la vez de los dos jugadores, y el mismo intento enviado dos veces; una partida jugada hasta el final con su único recibo de puntos; el intento ganador y el abandono del rival en el mismo instante; la revancha pedida por los dos; la arena con ocho disparando a la vez, reenvíos incluidos, sin que el código salga antes del final; el chat a ráfagas; ocho vestíbulos sondeando en cinco rondas; el código del día con ocho jugadores y ocho intentos cada uno; las peticiones malformadas; un turno de 30 s que vence con dos avisos simultáneos; el Cron pasando mientras se juega; y veinte rondas de intentos cruzados. Comprueba invariantes —una sola partida por revancha, un solo recibo por partida, ningún secreto fuera de una partida activa, intentos contados iguales a intentos guardados, ningún 500— y sale con 1 si algo falla. El 23 de septiembre: 73 comprobaciones, 0 anomalías. Enseñó algo del propio servidor que conviene saber: cuando los dos jugadores disparan a la vez puede entrar un intento de cada uno, en orden —el segundo reintenta tras el conflicto y para entonces ya es su turno—; es correcto, y la simulación lo acepta a sabiendas. Tarda unos dos minutos, casi todos esperando a que venza el turno de 30 s.
...
| **L8 · Recepción y 5.0.0** | Relectura completa en tres idiomas a 375 px y en ordenador, retirada de los alias de Mesa, documento maestro al día, etiqueta `v5.0.0` | 5.0.0 | **Cerrado** el 23-09-2026 |

**La serie está cerrada.** Los ocho lotes se entregaron entre el 22 y el 23 de septiembre de 2026, cada uno comitado, empujado, desplegado y con su etiqueta. Lo que se quedó fuera a propósito, porque pedía lecturas nuevas en D1 que ningún lote quiso añadir sin medir: la gente conectada y el podio junto al vestíbulo del ordenador (L5), el puesto de la temporada y la presencia en el perfil, la racha en la tarjeta de fin (L6) y la barra de temporada que se llena junto a los puntos (L7). Son las candidatas naturales de la 5.1.

Diario de transiciones:
- 23-09-2026 · L8 · À faire → Cerrado · 5.0.0 · 321 pruebas en verde. Relectura hecha contra el servidor local con un barrido automático de las catorce pantallas de consulta —portada, vestíbulo, crear, práctica, código del día, enigmas con uno abierto, ranking, perfil, rivales, historial, reglas, cuenta, abrir arena y buzón— en español, inglés y francés, a 375, 900 y 1280 px, buscando la página que se desborda de lado y cada elemento cuyo contenido se sale de su caja; y la práctica en juego, en los tres idiomas. Encontró una sola cosa, de antes de la serie: el widget de Turnstile mide 300 px y no encoge, y en un teléfono de 375 la tarjeta del buzón le dejaba 295; ahora la tarjeta le presta su margen. El vestíbulo, revisado de noche y en inglés tras retirar los alias de Mesa. Del catálogo de idiomas salieron treinta y una claves que ya no pedía nadie —restos del acceso en dos pasos, de las columnas del diario de la 4.4.0, del ranking y del perfil de antes de la serie—, en los tres idiomas; se quedaron las diez del acceso antiguo que todavía fija `test/login-two-steps.test.js`.
...
- **5.0.0** — Cierre de la serie 5: la identidad Plaza completa. Relectura de todas las pantallas en español, inglés y francés, en el teléfono y en el ordenador; el buzón deja sitio al widget antispam en un teléfono de 375 px; se retiran los nombres de color de la época Mesa y treinta y una frases que ya no se enseñaban en ninguna parte; el documento maestro cuenta la serie entera.
